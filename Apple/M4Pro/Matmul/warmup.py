...
def main(device):
    ...
    warmup_A = torch.rand(1024, 1024, dtype=torch.float16)
    warmup_B = torch.rand(1024, 1024, dtype=torch.float16) 
    warmup_dict = {'A': warmup_A, 'B': warmup_B}

    print("[INFO] Loading warmup model...")
    if device == "ane":
        wu_model = ct.models.MLModel('matmul1024_model_fp16.mlpackage', compute_units=ct.ComputeUnit.CPU_AND_NE)
    elif device == "gpu":
        wu_model = ct.models.MLModel('matmul1024_model_fp16.mlpackage', compute_units=ct.ComputeUnit.CPU_AND_GPU)
    elif device == "cpu":
        wu_model = ct.models.MLModel('matmul1024_model_fp16.mlpackage', compute_units=ct.ComputeUnit.CPU_ONLY)
    elif device == "all":
        wu_model = ct.models.MLModel('matmul1024_model_fp16.mlpackage', compute_units=ct.ComputeUnit.ALL)
