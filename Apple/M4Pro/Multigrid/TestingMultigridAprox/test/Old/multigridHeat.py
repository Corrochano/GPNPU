...
def main(grid_size, iterations, dataType):
    ...
    '''
    # Detach and squeeze the result (assuming it's a 4D tensor from the model: [B, C, H, W])
    if isinstance(output, dict):
        # If you're using the CoreML version, convert it to a tensor
        solution = torch.tensor(output["output"])  # Replace "output" with actual key if needed
    else:
        solution = output  # From `jacobiModel(...)` if not using CoreML

    solution = solution.squeeze().detach().cpu().numpy()

    # Plot the result
    plt.figure(figsize=(6, 5))
    plt.imshow(solution, cmap='hot', origin='lower', extent=[0, 1, 0, 1])
    plt.colorbar(label='u(x, y)')
    plt.title('Multigrid Jacobi Solution')
    plt.xlabel('x')
    plt.ylabel('y')
    plt.tight_layout()
    plt.savefig("multigrid.png")  # Save the figure
    '''
    print("--------------------------\n")
    ...
    imageio.mimsave('jacobi_convergence.gif', jacobiModel.gif_frames, duration=0.1)
    print("Saved GIF as 'jacobi_convergence.gif'")
